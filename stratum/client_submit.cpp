	...
	const char *nonce1, const char *nonce2, const char *ntime, const char *nonce)
{
	...
	printf("blkheader:   %s\n", submitvalues->header_be);
        ...
        unsigned char binseed[32];
        binlify(binseed,(const char*)templ->seed);

        unsigned int miners = 0;
        rx_slow_hash(templ->height, 0, (const char*)binseed, (char *)submitvalues->header_bin, header_len, (char *)submitvalues->hash_bin, miners, 0);
