...
void build_submit_values(YAAMP_JOB_VALUES *submitvalues, YAAMP_JOB_TEMPLATE *templ, const char *nonce1, const char *nonce2, const char *ntime, const char *nonce)
{
	...
	unsigned char coinbase_bin[4096];
	memset(coinbase_bin, 0, 4096);
	binlify(coinbase_bin, submitvalues->coinbase);

	char doublehash[256];
	memset(doublehash, 0, 256);
...
//	printf("Submit values header(%u)(%u): %s\n",strlen(submitvalues->header),strlen(submitvalues->header)/2, submitvalues->header);

	int header_len = strlen(submitvalues->header)/2;
	g_current_algo->hash_function((const char *)submitvalues->header_bin, (char *)submitvalues->hash_bin, header_len);
	...
	char *extranonce2, char *ntime, char *nonce, char *vote)
{
	...
	if(job->deleted) return;
        ...
        uint64_t coin_target = decode_compact(templ->nbits) / 65536;
	...
	if(block_hex==NULL) return;
//	memset(block_hex,0,block_size);

	// do aux first
	for(int i=0; i<templ->auxs_size; i++)
	{
		if(templ->auxs[i]==NULL) continue;

		YAAMP_COIND *coind_aux = templ->auxs[i]->coind;

		if(coind_aux==NULL) continue;

		if(!strcmp(coind->symbol, coind_aux->symbol2)) continue;

		unsigned char target_aux[1024] = { 0 };
		...
		if(hash_int <= coin_target_aux)
		{
			...
			if(b){
				...
				block_add(client->userid, client->workerid, coind_aux->id, coind_aux->height, target_to_diff(coin_target_aux),target_to_diff(hash_int), coind_aux->aux.hash, "", 0);
			}else{
        ...
        if(share_diff > coin_diff)
	{
		...
		if(b)
		{
			debuglog("*** ACCEPTED %s %d (diff %g) by %s (id: %d)\n", coind->name, templ->height,target_to_diff(hash_int), client->sock->ip, client->userid);
			...
			block_add(client->userid, client->workerid, coind->id, templ->height, target_to_diff(coin_target), target_to_diff(hash_int), hash1, submitvalues->hash_be, templ->has_segwit_txs);
...
//        debuglog("hash %016llx | target %016llx\n", hash_int, user_target);
